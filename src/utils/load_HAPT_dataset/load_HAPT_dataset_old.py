...
def load_HAPT_raw_data(DATA_DIR, TRAIN_SUBJECTS, ActID, window_size, overlap,
    ...
    Returns:
        ...
        act2label (Dict[str, int]): Dict of title_of_class to label_id
    """
    X_train, X_test, Y_train, Y_test = preprocess_raw_data(DATA_DIR, TRAIN_SUBJECTS, ActID,
                                                           window_size, overlap, scaler=scaler,
    ...
    return np.expand_dims(X_train, axis=1), np.expand_dims(X_test, axis=1), (y_train-1).squeeze(), (y_test-1).squeeze(), label2act, act2label

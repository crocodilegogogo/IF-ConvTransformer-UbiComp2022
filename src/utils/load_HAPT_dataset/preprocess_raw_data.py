...
# def preprocess_raw_data(DATA_DIR, TRAIN_SUBJECTS, ActID, window_size, overlap, scaler, separate_gravity_flag=False):
...
#             if separate_gravity_flag:
#                 pre_obj = Preprocess()
#                 acc_body, acc_grav = pre_obj.separate_gravity(temp_acc_raw) # seperate gravity from acc
...
#                               "GyroX", "GyroY", "GyroZ"]
#             # the number of windows in current experiment, user and action, put them into features
#             features = np.zeros((len(tBodyGyroXYZ), 128, channel_num)) # len(tAccXYZ):number of windows, 128:window size, 6:the axises of channels
#             # concatenate acc and gyro data into 'feature', then the seg windows of data are put into features
#             for i in range(len(tBodyGyroXYZ)):
#                 if separate_gravity_flag:
#                     np_concat = np.concatenate((tGravityAccXYZ[i], tBodyGyroXYZ[i], tBodyAccXYZ[i]), 1)

...
#         self.k, self.k_out, self.heads = k, k_out, heads
#         # input 维度为k（embedding结果），map成一个k*heads维度的矩阵
#         self.tokeys    = nn.Linear(k, k_out * heads, bias = False)
#         self.toqueries = nn.Linear(k, k_out * heads, bias = False)
#         self.tovalues  = nn.Linear(k, k_out * heads, bias = False)
#         # 设置dropout
#         self.dropout_attention = nn.Dropout(drop_rate)
#         # 在通过线性转换把维度压缩到 k
#         self.unifyheads = nn.Linear(heads * k_out, k)
...
#         k_out   = self.k_out
#         h = self.heads
#         queries = self.toqueries(x).view(b, t, h, k_out)
#         keys    = self.tokeys(x).view(b, t, h, k_out)
#         values  = self.tovalues(x).view(b, t, h, k_out)
#         # 把 head 压缩进 batch的dimension
#         queries = queries.transpose(1, 2).contiguous().view(b * h, t, k_out)
#         keys    = keys.transpose(1, 2).contiguous().view(b * h, t, k_out)
#         values  = values.transpose(1, 2).contiguous().view(b * h, t, k_out)
#         # 这等效于对点积进行normalize
#         queries = queries / (k_out ** (1/4))
#         keys = keys / (k_out ** (1/4))
#         # 矩阵相乘
#         dot  = torch.bmm(queries, keys.transpose(1,2))
#         # 进行softmax归一化
#         dot = F.softmax(dot, dim=2)
#         dot = self.dropout_attention(dot)
#         out = torch.bmm(dot, values).view(b, h, t, k_out)
#         # swap h, t back, unify heads
#         out = out.transpose(1, 2).contiguous().view(b, t, h*k_out)
        
#         return self.unifyheads(out) # (b, t, k_out)
...
#            kernel_size_acc  = padding_grav*2+1
    ...
    def forward(self, x):
        ...
        # norm_num     = sensor_attn.squeeze().reshape(sensor_attn.shape[0],-1)
...
#        out_attitude = sum_attitude
        ...
        # out_attitude = self.attitude_BN(out_attitude)
        # out_attitude = self.attitude_relu(out_attitude)
...
#            nn.Conv1d(feature_channel*(input_channel-reduced_channel), feature_channel_out, 1, 1),
    ...
    def forward(self, x):
        ...
        # x = x.reshape(x.shape[0], x.shape[1], -1, self.scale_num*3, x.shape[-1]).permute(0,1,3,2,4)
        # x = x.reshape(batch_size, x.shape[1]*x.shape[2], -1, data_length)
        # x = self.transition1(x)
        
        # # used
        # x = x.reshape(batch_size, -1, data_length).permute(0,2,1)
        # x = self.norm_conv4(x).permute(0,2,1)
        # x = x.reshape(batch_size, self.feature_channel, -1, data_length)
        
        # newly used
        x = x.permute(0, 3, 2, 1)
        x = self.norm_conv4(x).permute(0, 3, 2, 1)
        
        # x = x.reshape(batch_size, -1, IMU_num*2, data_length)
        # x = x.permute(0, 3, 2, 1).reshape(batch_size*data_length, IMU_num*2, -1)
        # x = self.sa(x).reshape(batch_size, data_length, IMU_num*2, -1)
        # x = x.permute(0, 3, 2, 1)
